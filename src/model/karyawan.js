...
const createKaryawan = (data) => {
  ...
  return new Promise((resolve, reject) => {
    pool.query(
      `INSERT INTO tbl_karyawan (id, nama, username, email, password, is_dev, is_manager, is_admin, is_resepsionis, is_perawat, is_dokter, is_manajemen, jenis_kelamin, nomor_kitas, tipe_izin, kadaluarsa_izin, nomor_hp, tempat_lahir, tanggal_lahir, alamat, status_menikah, created_at, updated_at) VALUES ('${id}', '${nama}', '${username}', '${email}', '${password}', '${is_dev}', '${is_manager}', '${is_admin}', '${is_resepsionis}', '${is_perawat}', '${is_dokter}', '${is_manajemen}', '${jenis_kelamin}', '${nomor_kitas}', '${tipe_izin}', '${kadaluarsa_izin}', '${nomor_hp}', '${tempat_lahir}', '${tanggal_lahir}', '${alamat}', '${status_menikah}', NOW(), NOW())`,
      (err, res) => {
        if (!err) {
          resolve(res);
        } else {
